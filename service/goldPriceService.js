    ...
    if (!data) {
      request(urlGTA, (error, response, html) => {
        if (!error && response.statusCode === 200) {
          ...
          let dateTime = $('#DetailPlace_uc_goldprices1_lblAsTime > b > font').text().split(' ');
          let date = dateTime[0].split('/');
          let update_time = `${dateTime[1]} ${dateTime[2]} ${dateTime[3]}`;
          let gold_sell = $('#DetailPlace_uc_goldprices1_lblOMSell > b > font').text();
          let gold_buy = $('#DetailPlace_uc_goldprices1_lblOMBuy > b > font').text();
          let goldBar_sell = $('#DetailPlace_uc_goldprices1_lblBLSell > b > font').text();
          let goldBar_buy = $('#DetailPlace_uc_goldprices1_lblBLBuy > b > font').text();
